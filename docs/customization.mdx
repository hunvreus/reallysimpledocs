...
Then load htmx from the installed package (or from a CDN) and use `bodyAttrs` and `Head` to boost links into the docs content container:
...
```astro title="src/docs/Head.astro (CDN)"
<script src="https://cdn.jsdelivr.net/npm/htmx.org@2.0.10/dist/htmx.min.js" defer></script>
```
</CodeGroup>

See the [htmx installation docs](https://htmx.org/docs/#installing) for the current package and CDN options.
