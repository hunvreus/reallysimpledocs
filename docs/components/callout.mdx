...
Use `Callout` for notes, warnings, and destructive or irreversible actions. The default callout uses regular Basecoat alert styling and has no icon. Warning and danger callouts use variant colors.
...
| --- | --- | --- | --- |
| `type` | `"warning"` \| `"danger"` \| `"destructive"` | `undefined` | Optional severity variant. |
| `variant` | `"warning"` \| `"danger"` \| `"destructive"` | `undefined` | Alias for `type`; wins when both are set. |
